...
def main(args):
    ...
    # TA's code, finish part 1 can show one face and one non face
    """
    ...
    y_test = []
    x = list(range(1, 11))
    # for t in range(1, 11):      # part 3 modify value at T from 1 to 10
    ...
    y_train.append(utils.evaluate(clf, trainData))

    print('\nEvaluate your classifier with test dataset')
    y_test.append(utils.evaluate(clf, testData))

    # part 3: plot chart to represent the training result
    """
    plt.plot(x, y_test, 'g')
    plt.plot(x, y_train, 'y')
    plt.ylim(0, 1)
    plt.title("Accuracy for each Iteration")    # title
    plt.xlabel("Iteration")     # x lable
    plt.ylabel("Accuracy")      # y lable
    plt.savefig('small ac chart')
    plt.show()
    """
