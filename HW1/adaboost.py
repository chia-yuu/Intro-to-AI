...
class Adaboost:
    ...
    def train(self, dataset):
        ...
        for t in range(self.T):
            ...
            # compute accuracy in each iteration
            # ac_list.append(utils.evaluate(clf, dataset))
        # return ac_list
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        explain part 2 code
        To find the best classifier, we iterate through all the features and possible polarity.
        In each iteration, we construct a WeakClassifier object and compute its error.
        If the classifier's result is not the same as lable, then add the weight to the error.
        If its error is smaller than the current bestError, then we update the bestError and bestClf.
        """
        # raise NotImplementedError("To be implemented")
        bestClf = None
        bestError = float('inf')
        for i in range(len(features)):
            # try -1, 1 in all features
            for polarity in [-1, 1]:
                clf = WeakClassifier(features[i], 0, polarity)
                error = 0.0
                # compute error
                for idx in range(len(labels)):
                    if clf.classify(iis[idx]) != labels[idx]:
                        error += weights[idx]
                # find smaller error, update bestError and bestClf
                if error<bestError:
                    bestError = error             # part 2
                    # bestError = error * error       # part 6
                    bestClf = clf
