...
class Agent():
    ...
    def learn(self):
        ...
        observations = torch.tensor(t_observations, dtype = torch.float)
        rewards = torch.tensor(t_rewards, dtype = torch.float)
        next_observations = torch.tensor(t_next_observations, dtype = torch.float)
    ...
    def check_max_Q(self):
        ...
        # TODO
        # raise NotImplementedError("Not implemented yet.")
        return torch.max(self.evaluate_net.forward(torch.tensor(self.env.reset(), dtype = torch.float)))
        # End your code
